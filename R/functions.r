...
filter_rawfolder <- function(raw_folders){
  ...
  raw_folders<-raw_folders[lapply(raw_folders,function(x) length(grep('S2018829_PKINGSBAY_2142/ACOUSTIC/EK80/EK80_ORIGINALRAWDATA',x,value=FALSE))) == 0]
...
copydata <- function(dir = '//ces.hi.no/mea/scratch/CRIMAC_survey_data',survey = NULL){
  ...
  copyRaw_data <- function(CES_path,Acoustic_dir,listoffiles=NULL){
...
#       #General case
        ...
        else if(grepl("EK60_RAADATA",raw,fixed=TRUE)){
  ...
  for (i in 1:nrow((survey))){
    ...
    #Copy raw data
    print('Copy raw data')
    copyRaw_data(CES_path,Acoustic_dir,listoffiles)
